...
from timm.models.layers import ClassifierHead, DropPath, AvgPool2dSame, get_act_layer, get_act_fn
#ScaledStdConv2d, ScaledStdConv2dSame, get_attn, make_divisible
...
def get_attn(attn_type):
    ...
    if attn_type is not None:
        if isinstance(attn_type, str):
            ...
            if attn_type == 'se':
                #from timm.models.layers.se import SEModule
                module_cls = SEModule
            elif attn_type == 'ese':
                ...
                from timm.models.layers.cbam import LightCbamModule
...
class NormFreeNet(nn.Module):
    ...
    def change_input(self, num_inputs):
        init_conv = self.stem.conv if 'conv' in dir(self.stem) else self.stem.conv1
        data = init_conv.weight.data
        ...
        print(init_conv.weight.data.shape, '->', data.shape)
        init_conv.weight.data = data
        
        return self
    
    def set_initial_stride(self, stride0):
        if 'conv' in dir(self.stem):
            self.stem.conv.stride = (stride0,stride0)
        if 'conv1' in dir(self.stem):
            self.stem.conv1.stride = (stride0,stride0)
        if 'pool' in dir(self.stem):
            self.stem.pool.stride = stride0
        if 'conv4' in dir(self.stem):
            self.stem.conv4.stride = (stride0,stride0)
        return self
